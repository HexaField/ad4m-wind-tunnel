...
ad4m_gql() {
    local query="$1"
    curl -sf -X POST "http://${AD4M_HOST}:${AD4M_PORT}/graphql" \
...
AGENT_RESULT=$(ad4m_gql 'mutation { agentGenerate(passphrase: "test passphrase") { did isInitialized } }')
AGENT_DID=$(echo "$AGENT_RESULT" | jq -r '.data.agentGenerate.did // empty' 2>/dev/null)
if [[ -z "$AGENT_DID" ]]; then
    AGENT_DID=$(ad4m_gql '{ agentStatus { did } }' | jq -r '.data.agentStatus.did // "unknown"' 2>/dev/null)
fi
...
PERSP_RESULT=$(ad4m_gql 'mutation { perspectiveAdd(name: "Flux Matrix Bridge") { uuid } }')
...
UNLOCK_RESULT=$(ad4m_gql 'mutation { agentUnlock(passphrase: "test passphrase", holochain: false) { did isUnlocked } }')
UNLOCK_DID=$(echo "$UNLOCK_RESULT" | jq -r '.data.agentUnlock.did // empty' 2>/dev/null)
...
if [[ "$BODY_COUNT" -gt 0 ]]; then
    ...
    if [[ "$LINK_COUNT" -gt 5 ]]; then
        ...
        echo "$LINKS_DATA" | jq -c '.[] | select(.data.predicate | startswith("flux://")) | .data' 2>/dev/null | head -5
...
]) { author data { source target predicate } } }"

BATCH_RESULT=$(ad4m_gql "$BATCH_GQL")
ADDED=$(echo "$BATCH_RESULT" | jq '.data.perspectiveAddLinks | length' 2>/dev/null) || ADDED=0
if [[ "$ADDED" -gt 0 ]]; then
