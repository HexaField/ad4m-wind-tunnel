...
#
# Requirements: Docker, Node.js (npx/tsx), Python3, jq, curl, sqlite3
#
# Usage:
#   ./matrix-bridge-test.sh                              # Full automated test
#   ./matrix-bridge-test.sh --skip-build --keep           # Skip build, keep services
#   ./matrix-bridge-test.sh --fresh --gql-port 13000     # Clean slate, custom port
set -euo pipefail
...
Port/Credential Options:
  --gql-port PORT         AD4M GraphQL port (default: 12100)
...
Examples:
  ...
  ./matrix-bridge-test.sh --ad4m ~/src/ad4m --gql-port 13000
EOF
    exit 0
}
...
GQL_URL="http://${AD4M_HOST}:${GQL_PORT}/graphql"
...
if lsof -ti:"$GQL_PORT" >/dev/null 2>&1; then
    warn "Port $GQL_PORT in use — killing existing process"
    lsof -ti:"$GQL_PORT" | xargs kill -9 2>/dev/null || true
    sleep 2
fi
...
if ! start_executor "$AD4M_EXECUTOR" "$DATA_DIR" "$SEED_PATH" "$GQL_PORT" "$CREDENTIAL"; then
    fail "executor-start" "Failed to start executor"
    print_summary "Matrix ↔ AD4M/Flux Bridge" || exit 1
fi
pass "executor-start" "Ready (PID $EXECUTOR_PID, port $GQL_PORT)"
...
PERSP_RESULT=$(ad4m_gql 'mutation { perspectiveAdd(name: "Matrix Bridge") { uuid } }')
PERSPECTIVE_UUID=$(echo "$PERSP_RESULT" | jq -r '.data.perspectiveAdd.uuid // empty' 2>/dev/null)
...
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"ad4m://self\", target: \"flux://has_community\", predicate: \"flux://entry_type\"}) { data { source } } }" >/dev/null 2>&1
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"ad4m://self\", target: \"literal://string:Matrix%20Bridge\", predicate: \"rdf://name\"}) { data { source } } }" >/dev/null 2>&1
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"ad4m://self\", target: \"$CHANNEL_ID\", predicate: \"flux://has_channel\"}) { data { source } } }" >/dev/null 2>&1
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"$CHANNEL_ID\", target: \"flux://has_channel\", predicate: \"flux://entry_type\"}) { data { source } } }" >/dev/null 2>&1
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"$CHANNEL_ID\", target: \"literal://string:general\", predicate: \"flux://has_channel_name\"}) { data { source } } }" >/dev/null 2>&1
...
UNLOCK_RESULT=$(ad4m_gql "mutation { agentUnlock(passphrase: \"$PASSPHRASE\", holochain: false) { did isUnlocked } }")
UNLOCK_OK=$(echo "$UNLOCK_RESULT" | jq -r '.data.agentUnlock.isUnlocked // false' 2>/dev/null)
...
PERSP_CHECK=$(ad4m_gql "{ perspective(uuid: \"$PERSPECTIVE_UUID\") { neighbourhood { data { linkLanguage } } } }")
LOADED_LANG=$(echo "$PERSP_CHECK" | jq -r '.data.perspective.neighbourhood.data.linkLanguage // empty' 2>/dev/null)
...
LINKS_RAW=$(ad4m_gql "{ perspectiveQueryLinks(uuid: \"$PERSPECTIVE_UUID\", query: {}) { data { source target predicate } } }")
LINKS_DATA=$(echo "$LINKS_RAW" | jq '.data.perspectiveQueryLinks // []' 2>/dev/null)
TOTAL_LINKS=$(echo "$LINKS_DATA" | jq 'length' 2>/dev/null) || TOTAL_LINKS=0
info "Total links in perspective: $TOTAL_LINKS"

BODY_LINKS=$(echo "$LINKS_DATA" | jq '[.[] | select(.data.predicate == "flux://body")]' 2>/dev/null) || BODY_LINKS="[]"
BODY_COUNT=$(echo "$BODY_LINKS" | jq 'length' 2>/dev/null) || BODY_COUNT=0

HAS_CHILD=$(echo "$LINKS_DATA" | jq '[.[] | select(.data.predicate == "ad4m://has_child")]' 2>/dev/null) || HAS_CHILD="[]"
...
if [[ "$BODY_COUNT" -gt 0 ]]; then
    ...
    if [[ "$TOTAL_LINKS" -gt 5 ]]; then
        ...
        echo "$LINKS_DATA" | jq -c '.[] | select(.data.predicate | startswith("flux://")) | .data' 2>/dev/null | head -5 | sed 's/^/    /'
        skip "matrix-to-ad4m" "Language active, sync still propagating"
    else
        fail "matrix-to-ad4m" "No flux://body links found"
        echo "  All links:"
        echo "$LINKS_DATA" | jq -c '.[] | .data' 2>/dev/null | head -10 | sed 's/^/    /'
...
BATCH_GQL="mutation {
    perspectiveAddLinks(uuid: \"$PERSPECTIVE_UUID\", links: [
...
BATCH_RESULT=$(ad4m_gql "$BATCH_GQL")
ADDED_COUNT=$(echo "$BATCH_RESULT" | jq '.data.perspectiveAddLinks | length' 2>/dev/null) || ADDED_COUNT=0
...
if [[ "$ADDED_COUNT" -gt 0 ]]; then
    ...
    ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"$CHANNEL_ID\", target: \"$FLUX_MSG_ID\", predicate: \"ad4m://has_child\"}) { data { source } } }" >/dev/null 2>&1
    ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"$FLUX_MSG_ID\", target: \"flux://has_message\", predicate: \"flux://entry_type\"}) { data { source } } }" >/dev/null 2>&1
    ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"$FLUX_MSG_ID\", target: \"$ENCODED_BODY\", predicate: \"flux://body\"}) { data { source } } }" >/dev/null 2>&1
