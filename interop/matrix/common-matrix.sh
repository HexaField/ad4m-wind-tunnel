...
#
# Provides Matrix API helpers, AD4M GraphQL helpers, Flux seed generation,
...
matrix_create_room() {
    ...
    if [[ -z "$room_id" && -n "$alias" ]]; then
        ...
        # Try to extract server_name from a whoami or versions call; fallback to URL host
        local encoded_alias
...
# Execute a GraphQL query/mutation against the AD4M executor.
# Requires AD4M_HOST, GQL_PORT, CREDENTIAL to be set.
# Usage: ad4m_gql "QUERY_STRING"
ad4m_gql() {
    local query="$1"
    local gql_url="http://${AD4M_HOST:-127.0.0.1}:${GQL_PORT:-12100}/graphql"
    curl -sf -X POST "$gql_url" \
        -H "Content-Type: application/json" \
        -H "Authorization: ${CREDENTIAL:-test123}" \
        -d "{\"query\": $(echo "$query" | jq -Rs '.')}" 2>/dev/null || echo ""
}
...
# Usage: start_executor AD4M_EXECUTOR DATA_DIR SEED_PATH GQL_PORT CREDENTIAL
# Sets EXECUTOR_PID in the caller's scope.
start_executor() {
    local executor="$1" data_dir="$2" seed_path="$3" gql_port="$4" credential="$5"
    ...
    step "Starting AD4M executor (language-language-only, port $gql_port)..."
    "$executor" run \
    ...
    # Wait for GraphQL
    local gql_url="http://127.0.0.1:${gql_port}/graphql"
    local ready=false
    ...
    if [[ "$ready" == "true" ]]; then
        info "Executor ready (PID $EXECUTOR_PID, port $gql_port)"
        return 0
    else
...
publish_language() {
    ...
    gql="mutation {
        languagePublish(
            languagePath: \"$bundle_path\",
            languageMeta: {
                name: \"matrix-link-language\",
                description: \"Matrix bridge link language for Flux interop\",
                possibleTemplateParams: [\"MATRIX_HOMESERVER_URL\",\"MATRIX_ROOM_ID\",\"MATRIX_USER_ID\",\"MATRIX_ACCESS_TOKEN\",\"MATRIX_ROOM_ALIAS\",\"NEIGHBOURHOOD_META\"]
            }
        ) { address name }
    }"
    ...
    result=$(ad4m_gql "$gql")
    local hash
    hash=$(echo "$result" | jq -r '.data.languagePublish.address // empty' 2>/dev/null)
    ...
    if [[ -z "$hash" ]]; then
        gql="mutation {
            languagePublish(
                languagePath: \"$bundle_path\",
                languageMeta: { name: \"matrix-link-language\", description: \"Matrix bridge link language\" }
            ) { address name }
        }"
        result=$(ad4m_gql "$gql")
        hash=$(echo "$result" | jq -r '.data.languagePublish.address // empty' 2>/dev/null)
...
apply_language_template() {
    ...
    gql="mutation {
        languageApplyTemplateAndPublish(
            sourceLanguageHash: \"$lang_hash\",
            templateData: $(echo "$template_data" | jq -Rs '.')
        ) { address name }
    }"
    ...
    result=$(ad4m_gql "$gql")
    local configured
    configured=$(echo "$result" | jq -r '.data.languageApplyTemplateAndPublish.address // empty' 2>/dev/null)

    # Fallback mutation name
    if [[ -z "$configured" ]]; then
        gql="mutation {
            languageApplyTemplate(
                sourceLanguageHash: \"$lang_hash\",
                templateData: $(echo "$template_data" | jq -Rs '.')
            ) { address }
        }"
        result=$(ad4m_gql "$gql")
        configured=$(echo "$result" | jq -r '.data.languageApplyTemplate.address // empty' 2>/dev/null)
    fi
...
init_or_unlock_agent() {
    local passphrase="$1"
    local status_result
    status_result=$(ad4m_gql '{ agentStatus { isInitialized isUnlocked did } }')
    local is_init is_unlocked agent_did

    is_init=$(echo "$status_result" | jq -r '.data.agentStatus.isInitialized // false' 2>/dev/null)
    is_unlocked=$(echo "$status_result" | jq -r '.data.agentStatus.isUnlocked // false' 2>/dev/null)
    agent_did=$(echo "$status_result" | jq -r '.data.agentStatus.did // empty' 2>/dev/null)

    if [[ "$is_init" != "true" ]]; then
        step "Generating new agent..." >&2
        local gen_result
        gen_result=$(ad4m_gql "mutation { agentGenerate(passphrase: \"$passphrase\") { did isInitialized } }")
        agent_did=$(echo "$gen_result" | jq -r '.data.agentGenerate.did // empty' 2>/dev/null)
        if [[ -z "$agent_did" ]]; then
        ...
        agent_did=$(echo "$unlock_result" | jq -r '.data.agentUnlock.did // empty' 2>/dev/null)
        if [[ -z "$agent_did" ]]; then
