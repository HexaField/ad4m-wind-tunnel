...
success "Executor ready (PID $EXECUTOR_PID, port $GQL_PORT)"
...
PERSP_RESULT=$(ad4m_gql "mutation { perspectiveAdd(name: \"$COMMUNITY_NAME\") { uuid } }")
...
ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"ad4m://self\", target: \"$ENCODED_NAME\", predicate: \"rdf://name\"}) { data { source } } }" >/dev/null 2>&1
...
if [[ -n "$ROOM_TOPIC" ]]; then
    ...
    ad4m_gql "mutation { perspectiveAddLink(uuid: \"$PERSPECTIVE_UUID\", link: {source: \"ad4m://self\", target: \"$ENCODED_TOPIC\", predicate: \"rdf://description\"}) { data { source } } }" >/dev/null 2>&1
fi
...
ad4m_gql "mutation { agentUnlock(passphrase: \"$PASSPHRASE\", holochain: false) { did isUnlocked } }" >/dev/null 2>&1
...
if [[ "$BACKFILL" -gt 0 ]]; then
    ...
    echo "$HISTORY" | jq -c '.chunk[]? | select(.type=="m.room.message") | {sender: .sender, body: .content.body, ts: .origin_server_ts}' 2>/dev/null | while read -r msg_json; do
        ...
        if [[ -n "$local_body" ]]; then
            ...
            ad4m_gql "mutation { perspectiveAddLinks(uuid: \"$PERSPECTIVE_UUID\", links: [
...
if [[ -d "$FLUX_DIR/app/dist" ]]; then
    ...
    CREATE_USER=$(ad4m_gql "mutation { runtimeCreateUser(email: \"$FLUX_EMAIL\", password: \"$FLUX_PASS\") { jwt did } }")
    USER_JWT=$(echo "$CREATE_USER" | jq -r '.data.runtimeCreateUser.jwt // empty' 2>/dev/null)
    if [[ -z "$USER_JWT" ]]; then
        LOGIN_USER=$(ad4m_gql "mutation { runtimeLoginUser(email: \"$FLUX_EMAIL\", password: \"$FLUX_PASS\") { jwt did } }")
...
echo ""
echo "  AD4M GraphQL:    http://127.0.0.1:${GQL_PORT}/graphql"
